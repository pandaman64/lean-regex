...
def materializeUpdatesAux (n : Nat) (accum : Vector (ValidPosPlusOne s) n) : List (Nat × Pos s) → Vector (ValidPosPlusOne s) n
  | [] => accum
  | (offset, pos) :: rest =>
...
one wins.
-/
def materializeUpdates (n : Nat) (updates : List (Nat × Pos s)) : Vector (ValidPosPlusOne s) n :=
  materializeUpdatesAux n (Vector.replicate n (.sentinel s)) updates

def equivPos {s} (p : Option (Pos s)) (p' : ValidPosPlusOne s) : Prop :=
  match p with
  | .some p => p' = .validPos p
  | .none => p' = .sentinel s

@[simp, grind =]
theorem equivPos.some_iff {p : Pos s} {p' : ValidPosPlusOne s} : equivPos (.some p) p' ↔ p' = .validPos p := by
  grind [equivPos]
...
@[simp, grind =]
theorem equivPos.validPos_iff {p p' : Pos s} : equivPos p (.validPos p') ↔ p = p' := by
  grind [equivPos]
...
def EquivMaterializedUpdate {n} (groups : Nat → Option (Pos s × Pos s)) (updates : Vector (ValidPosPlusOne s) n) : Prop :=
  ∀ tag,
    ((h₁ : 2 * tag < n) → (equivPos ((groups tag).map (·.1)) updates[2 * tag])) ∧
    ((h₂ : 2 * tag + 1 < n) → (equivPos ((groups tag).map (·.2)) updates[2 * tag + 1]))

theorem EquivMaterializedUpdate.eq {n} {groups : Nat → Option (Pos s × Pos s)} {updates : Vector (ValidPosPlusOne s) n}
...
theorem EquivMaterializedUpdate.eq_none {n} {groups : Nat → Option (Pos s × Pos s)} {updates : Vector (ValidPosPlusOne s) n}
