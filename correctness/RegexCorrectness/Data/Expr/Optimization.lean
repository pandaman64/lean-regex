...
open Std in
theorem curr_of_captures_of_firstChars_some {s} {p p' : ValidPos s} {groups e n cs}
  (c : Expr.Captures p p' groups e) (h : e.firstChars n = .some cs) (ne : p ≠ s.endValidPos) :
  ...
  | alternateLeft c₁ ih =>
    ...
    simpa [←hcs, HashSet.mem_union_iff] using .inl (ih ne hcs₁)
  | alternateRight _ ih =>
    intro cs h
    simp only [firstChars, HashSet.union_eq, Option.pure_def, Option.bind_eq_bind,
      Option.bind_eq_some_iff, Option.some.injEq] at h
    have ⟨_, _, _, hcs₂, hcs⟩ := h
    simpa [← hcs, HashSet.mem_union_iff] using .inr (ih ne hcs₂)
  | concat c₁ _ ih₁ ih₂ =>
    intro cs h
