...
variable {s : String} {σ : Strategy s} {nfa : NFA} {wf : nfa.WellFormed}
  {startPos : Pos s} {stack : List (StackEntry σ nfa startPos)} {update : σ.Update} {state : Fin nfa.size} {pos : BVPos startPos}
...
theorem fun_cases' (σ : Strategy s) (nfa : NFA) (wf : nfa.WellFormed) (startPos : Pos s)
  {motive : List (StackEntry σ nfa startPos) → σ.Update → Fin nfa.size → BVPos startPos → Prop}
  (done : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos),
    nfa[state] = .done → motive stack update state pos)
  (fail : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos),
    nfa[state] = .fail → motive stack update state pos)
  (epsilon : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (state' : Fin nfa.size),
    nfa[state] = .epsilon state' → motive stack update state pos)
  (split : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (state₁ state₂ : Fin nfa.size),
    nfa[state] = .split state₁ state₂ → motive stack update state pos)
  (save : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (offset : Nat) (state' : Fin nfa.size),
    nfa[state] = .save offset state' → motive stack update state pos)
  (anchor_pos : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (a : Data.Anchor) (state' : Fin nfa.size),
    nfa[state] = .anchor a state' → a.test pos.current → motive stack update state pos)
  (anchor_neg : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (a : Data.Anchor) (state' : Fin nfa.size),
    nfa[state] = .anchor a state' → ¬a.test pos.current → motive stack update state pos)
  (char_pos : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (c : Char) (state' : Fin nfa.size),
    nfa[state] = .char c state' → (h : pos ≠ s.endBVPos startPos) → pos.get h = c → motive stack update state pos)
  (char_neg : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (c : Char) (state' : Fin nfa.size),
    nfa[state] = .char c state' → (pos = s.endBVPos startPos ∨ ∃ ne : pos ≠ s.endBVPos startPos, pos.get ne ≠ c) → motive stack update state pos)
  (sparse_pos : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (cs : Data.Classes) (state' : Fin nfa.size),
    nfa[state] = .sparse cs state' → (h : pos ≠ s.endBVPos startPos) → pos.get h ∈ cs → motive stack update state pos)
  (sparse_neg : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (cs : Data.Classes) (state' : Fin nfa.size),
    nfa[state] = .sparse cs state' → (pos = s.endBVPos startPos ∨ ∃ ne : pos ≠ s.endBVPos startPos, pos.get ne ∉ cs) → motive stack update state pos)
  : ∀ (stack : List (StackEntry σ nfa startPos)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos),
...
theorem captureNextAux.induct' {s : String} (σ : Strategy s) (nfa : NFA) (wf : nfa.WellFormed) (startPos : Pos s)
  ...
  (done : ∀ (visited : BitMatrix nfa.size (startPos.remainingBytes + 1)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (stack' : List (StackEntry σ nfa startPos)),
    ¬visited.get state pos.index →
    nfa[state] = .done →
    motive visited (⟨update, state, pos⟩ :: stack'))
  (next : ∀ (visited : BitMatrix nfa.size (startPos.remainingBytes + 1)) (update : σ.Update) (state : Fin nfa.size) (pos : BVPos startPos) (stack' : List (StackEntry σ nfa startPos)),
    ¬visited.get state pos.index →
    nfa[state] ≠ .done →
    motive (visited.set state pos.index) (pushNext σ nfa wf startPos stack' update state pos) →
    motive visited (⟨update, state, pos⟩ :: stack'))
  : ∀ (visited : BitMatrix nfa.size (startPos.remainingBytes + 1)) (stack : List (StackEntry σ nfa startPos)), motive visited stack :=
...
variable {s : String} {σ : Strategy s} {nfa : NFA} {wf : nfa.WellFormed} {startPos : Pos s} {visited : BitMatrix nfa.size (startPos.remainingBytes + 1)}
...
theorem captureNext.go.induct' {s : String} (σ : Strategy s) (nfa : NFA) (wf : nfa.WellFormed) (startPos : Pos s)
  ...
  (found : ∀ (pos : BVPos startPos) (visited : BitMatrix nfa.size (startPos.remainingBytes + 1)) (update : σ.Update) (visited' : BitMatrix nfa.size (startPos.remainingBytes + 1)),
