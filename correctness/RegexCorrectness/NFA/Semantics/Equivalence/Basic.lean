...
inductive EquivUpdate {s : String} : CaptureGroups s → List (Nat × Pos s) → Prop where
...
theorem EquivUpdate.mem_groups_of_mem_updates {s : String} {groups : CaptureGroups s} {updates : List (Nat × Pos s)} {offset : Nat} {pos : Pos s}
