...
inductive VMPath (nfa : NFA) (wf : nfa.WellFormed) (it : Iterator) : Iterator → Fin nfa.nodes.size → List (Nat × Pos) → Prop where
  | init {i update} (cls : nfa.εClosure' it ⟨nfa.start, wf.start_lt⟩ i update) :
    VMPath nfa wf it it i update
  | more {i j k it' update₁ update₂} (prev : VMPath nfa wf it it' i update₁) (step : nfa.CharStep it' i j) (cls : nfa.εClosure' it'.next j k update₂) :
    VMPath nfa wf it it'.next k (update₁ ++ update₂)
...
TODO: I don't understand how this should be updated for Iterators.
-/
