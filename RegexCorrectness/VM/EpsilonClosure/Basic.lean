...
abbrev εStack' (nfa : NFA) := List (List (Nat × String.Pos) × Fin nfa.nodes.size)

/--
A version of εClosure traversal that tracks the updates to the capture groups.

As the NFA semantics use `List (Nat × String.Pos)` to represent the capture group updates, this
...
def εClosure' (nfa : NFA) (wf : nfa.WellFormed) (pos : Pos)
  (next : SparseSet nfa.nodes.size) (matched : Option (List (Nat × String.Pos))) (updates : Vec (List (Nat × String.Pos)) nfa.nodes.size)
  (stack : εStack' nfa) :
  ...
  | (update, state) :: stack' =>
    if state ∈ next then
      εClosure' nfa wf pos next matched updates stack'
    else
      let next' := next.insert state
      match hn : nfa[state] with
      | .epsilon state' =>
        have isLt : state' < nfa.nodes.size := wf.inBounds' state hn
        εClosure' nfa wf pos next' matched updates ((update, ⟨state', isLt⟩) :: stack')
      | .split state₁ state₂ =>
        have isLt : state₁ < nfa.nodes.size ∧ state₂ < nfa.nodes.size := wf.inBounds' state hn
        εClosure' nfa wf pos next' matched updates ((update, ⟨state₁, isLt.1⟩) :: (update, ⟨state₂, isLt.2⟩):: stack')
      | .save offset state' =>
        have isLt : state' < nfa.nodes.size := wf.inBounds' state hn
        let update' := update ++ [(offset, pos)]
        εClosure' nfa wf pos next' matched updates ((update', ⟨state', isLt⟩) :: stack')
      | .done =>
        let matched' := matched <|> update
        let buffers' := updates.set state state.isLt update
        εClosure' nfa wf pos next' matched' buffers' stack'
      | .char c state' =>
        let buffers' := updates.set state state.isLt update
        εClosure' nfa wf pos next' matched buffers' stack'
      | .sparse cs state' =>
        let buffers' := updates.set state state.isLt update
        εClosure' nfa wf pos next' matched buffers' stack'
      | .fail => εClosure' nfa wf pos next' matched updates stack'
termination_by (next.measure, stack)

-- Tidied up version of the function induction
theorem εClosure'.induct' (nfa : NFA) (wf : nfa.WellFormed) (pos : Pos)
  ...
  (save : ∀ next matched updates update state stack' offset state',
    ...
    motive (next.insert state) matched updates ((update ++ [(offset, pos)], state') :: stack') →
    motive next matched updates ((update, state) :: stack'))
  ...
  refine εClosure'.induct nfa wf pos motive base visited ?epsilon ?split ?save ?done ?char ?sparse ?fail
  case epsilon =>
...
variable {nfa wf pos next matched updates}

@[simp]
theorem εClosure'_base : εClosure' nfa wf pos next matched updates [] = (next, matched, updates) := by
  simp [εClosure']

@[simp]
theorem εClosure'_visited {update state stack'} (hmem : state ∈ next) :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos next matched updates stack' := by
  simp [εClosure', hmem]

@[simp]
theorem εClosure'_epsilon {update state stack' state'} (hmem : state ∉ next) (hn : nfa[state] = .epsilon state') :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) matched updates ((update, state') :: stack') := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
...
theorem εClosure'_split {update state stack' state₁ state₂} (hmem : state ∉ next) (hn : nfa[state] = .split state₁ state₂) :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) matched updates ((update, state₁) :: (update, state₂) :: stack') := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
...
theorem εClosure'_save {update state stack' offset state'} (hmem : state ∉ next) (hn : nfa[state] = .save offset state') :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) matched updates ((update ++ [(offset, pos)], state') :: stack') := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
...
theorem εClosure'_done {update state stack'} (hmem : state ∉ next) (hn : nfa[state] = .done) :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) (matched <|> some update) (updates.set state state.isLt update) stack' := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
...
theorem εClosure'_char {update state stack' c state'} (hmem : state ∉ next) (hn : nfa[state] = .char c state') :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) matched (updates.set state state.isLt update) stack' := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
...
theorem εClosure'_fail {update state stack'} (hmem : state ∉ next) (hn : nfa[state] = .fail) :
  εClosure' nfa wf pos next matched updates ((update, state) :: stack') = εClosure' nfa wf pos (next.insert state) matched updates stack' := by
  conv =>
    lhs
    unfold εClosure'
    simp [hmem]
