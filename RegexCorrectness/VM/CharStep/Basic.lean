...
/--
If the given state can make a transition on the given character, make the transition and traverse
ε-closures from the resulting state. `pos` should be the position after the given character.
-/
def stepChar' (nfa : NFA) (wf : nfa.WellFormed) (c : Char) (pos : Pos)
  ...
  | .char c' state' =>
    if c = c' then
      ...
      εClosure' nfa wf pos next .none updates [(update, ⟨state', isLt⟩)]
    else
      (next, .none, updates)
  | .sparse cs state' =>
    if c ∈ cs then
...
/--
For all states in `current`, make a transition on the given character and traverse ε-closures from
...
where
  go (i : Nat) (hle : i ≤ current.count) (next : SparseSet nfa.nodes.size) (updates : Vec (List (Nat × Pos)) nfa.nodes.size) :
    ...
    if h : i = current.count then
      ...
      let result := stepChar' nfa wf c pos next updates current[i]
      if result.2.1.isSome then
        result
      else
        go (i + 1) hlt result.1 result.2.2

theorem eachStepChar'.go.induct' (nfa : NFA) (wf : nfa.WellFormed) (c : Char) (p : Pos)
  ...
  (found : ∀ i next updates hlt next' matched updates',
    stepChar' nfa wf c p next updates current[i] = (next', matched, updates') →
  ...
  refine eachStepChar'.go.induct nfa wf c p current motive ?base ?found ?not_found i hle next updates
  case base =>
