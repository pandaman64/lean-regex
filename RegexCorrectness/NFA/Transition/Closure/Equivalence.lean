...
theorem eq_or_pathIn'_of_εClosure {nfa : NFA} (cls : i' ∈ nfa.εClosure i) :
...
theorem pathIn'.of_snoc_char (assm : cs' = cs ++ [c])
  ...
  | @more i j k cs₁ cs₂ step rest ih =>
    match pathIn.of_snoc_char.snoc_eq_append assm.symm with
    | .inl ⟨h₁, h₂⟩ =>
      simp [h₁, h₂] at step rest
      have := step.nil_of_snoc
      simp [this] at *
...
theorem eq_or_pathIn'_of_reaches {nfa : NFA} {i : Fin nfa.nodes.size} {m : List Char}
  (h : nfa.reaches i m) :
  (nfa.start = i ∧ m = []) ∨ nfa.pathIn' 0 nfa.start i m := by
  induction h with
  | nil cls =>
    cases eq_or_pathIn'_of_εClosure cls with
...
theorem matches_of_reaches' (eq : compile r = nfa)
  ...
  r.matches cs := by
  have hi : i.val = 0 := (done_iff_zero_compile eq i).mp h₂
  have path := eq_or_pathIn'_of_reaches h₁
  have : nfa.start ≠ i := by
