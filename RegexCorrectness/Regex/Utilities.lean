...
theorem captures_of_mem_findAll.go {m : Matches} {accum : Array (Pos × Pos)}
  (v : m.Valid) (inv : ∀ positions ∈ accum, Matches.Spec v.1 positions) :
  ∀ positions ∈ findAll.go m accum, Matches.Spec v.1 positions := by
  ...
  | case1 m accum positions m' next_some? ih =>
    ...
    have inv' (p₁ p₂ : Pos) (mem : (p₁, p₂) ∈ accum.push positions) : Matches.Spec v.1 (p₁, p₂) := by
...
theorem captures_of_mem_captureAll.go {captures : Captures} {accum : Array CapturedGroups}
  ...
  | case1 captures accum groups captures' next?_some ih =>
    ...
    have inv' (captured : CapturedGroups) (mem : captured ∈ accum.push groups) : captured.Spec v.1 := by
      simp [Array.push_eq_append_singleton] at mem
      cases mem with
